...
function ModuleRunner.new(options)
    ---@type ModuleRunner
    local self = setmetatable({}, ModuleRunner)
    self.evaluatedModules = options.evaluatedModules
    return self
end
...
---@return table
local function createIsolatedPackageLoaded(moduleRunner)
    local evaluatedModules = moduleRunner.evaluatedModules
...
---@param moduleRunner ModuleRunner
---@return table
local function createIsolatedEnv(moduleRunner)
    -- TODO: 我们需要更严格环境隔离
    ...
    local isolatedLoaded = createIsolatedPackageLoaded(moduleRunner)
...
function ModuleRunner:import(path)
    local fileEnv = createIsolatedEnv(self)
